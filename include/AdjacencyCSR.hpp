...
struct AdjacencyCSR {
    ...
    static inline uint32_t upack_id(const uint32_t packed) {
        return packed >> 2;
    }
    static inline uint32_t upack_order(const uint32_t packed) {
    ...
    inline uint32_t incident_packed(VertexId v, uint32_t k) const { return incidents[begin(v) + k]; }
